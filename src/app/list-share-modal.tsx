...
const APP_ACTIONS = [
  { id: 'copy',      label: 'Copy link',  sf: 'link' },
  { id: 'message',   label: 'Message',    sf: 'message' },
  { id: 'more',      label: 'More',       sf: 'ellipsis' },
] as const;
...
export default function ListShareModal() {
  const { listId, listTitle } = useLocalSearchParams<{ listId: string; listTitle: string }>();
  const Brand = useBrand();
  const styles = useMemo(() => createStyles(Brand), [Brand]);

  const url = `https://clique.app/list/${listId}`;

  async function handleAction(id: typeof APP_ACTIONS[number]['id']) {
    if (id === 'copy') {
      // Clipboard import avoided — use Share sheet to "copy" via the system
      router.back();
      setTimeout(async () => {
      ...
      setTimeout(async () => {
        await Share.share({
          title: listTitle,
          message: `Check out "${listTitle}" on Clique: ${url}`,
          url,
        });
      }, 300);
    } else if (id === 'more') {
